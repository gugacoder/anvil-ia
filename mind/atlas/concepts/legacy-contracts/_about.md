...
- **Forma**: tabelas markdown com colunas `Item | Tipo | Obrigatório | Semântica | Valores legais | Efeito | Vem de`.
- **Escopo**: tabelas SQL (`TB*`), procedures (`*.sp_*`, `*.obter_*`), atributos XML em campos `DFconfig`, formatos JSON expostos pelas APIs do `.NET`.
- **Audiência**: [[smith]] (implementador) e [[designer]] (catalogador de UX) consomem esses contratos. [[ui-tester]] usa como expectativa de teste.
- **Proibido**: dizer "use shadcn Table" ou "implemente com TanStack". O contrato fala da **linguagem do banco**, não da stack escolhida.
- **Citação obrigatória**: cada contrato cita o arquivo-fonte (`sources/...path/file.sql:linha`) pra rastreabilidade.
