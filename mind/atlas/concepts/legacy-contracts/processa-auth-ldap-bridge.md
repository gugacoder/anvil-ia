...
| `loginUser` | string | identity completa **com domain** (ex.: `"processa\\guga"`) | sAMAccountName + domain — bridge faz o split internamente | `AbstractBearerAuth.cs:109` (passa `user` cru) |
...
| Validação de grupo falha mas credencial OK | desconhecido — comportamento exato do bridge para `validarGrupo` setado + user válido não verificável sem credencial real | — | > **inferido**: provavelmente devolve `sucesso=true` com `dados=[]` (lista vazia) e cabe ao cliente decidir, OU `sucesso=false` com mensagem específica. Probe seguro impossível sem `processa\guga` real (proibido por mandato) |

### Discriminação `IsLdapUser` (cliente decide se aciona bridge)

| Identity de entrada | É LDAP? | Decisão |
|---|---|---|
| `processa\guga` | sim | chama bridge |
| `processa.com\guga` | sim | chama bridge |
| `Processa\Guga` | sim (case-insensitive) | chama bridge |
| `guga` (sem `\`) | não | cai em `AuthenticateLocal` (`AuthMiddleware`) ou em `AuthenticationException("Autenticação permitida somente para usuários internos.")` (`LDAPAuthMiddleware`) |
...
|---|---|---|---|---|
| A1 | identity contém `\` e prefixo case-insensitive `processa\` ou `processa.com\` (ex.: `processa\guga`, `Processa.com\Guga`) | path resolvido = `ldap-bridge` (chama `AuthenticateLdap`) | `User.StartsWith("processa\\", IgnoreCase) \|\| User.StartsWith("processa.com\\", IgnoreCase)` | `Processa.Sdk.Auth/AbstractBearerAuth.cs:91-92` |
...
| A3 | identity sem `\` que não é literal `processa` (ex.: `guga`) | `LDAPAuthMiddleware` → throw `AuthenticationException("Autenticação permitida somente para usuários internos.")`; `AuthMiddleware` (com fallback) → tenta `AuthenticateLocal` | `IsLdapUser()` retorna `false` por ausência do prefixo | `Processa.Sdk.Auth/LDAPAuthMiddleware.cs:36-39` + `AuthMiddleware.cs:36` |
...
| A20 | path canônico observável (assertion auditável end-to-end pelo ui-tester com `processa\guga`) | (a) Studio recebe `Basic base64("processa\\guga:<senha>")` em `POST /api/auth/login`; (b) detecta prefixo `processa\` (case-insensitive) → resolve para path `ldap-bridge`; (c) POSTa body `"<base64>"` ao bridge; (d) recebe 200 + `{sucesso:true, dados:[grupos]}`; (e) emite cookie `director_session` com principal `{id:1, nome:"processa\\guga", codEmpresa:1, nomeEmpresa:"Processa", grupos:[...]}` e responde 200 ao cliente. Falha de credencial em (d) → resposta 401 ao cliente com `{error:"invalid-credentials"}`. Falha de rede em (c) → 502 com `{error:"aws-unreachable"}` (host:port sanitizado da message) | composição A1+A4..A18 | (consolida fontes acima) |
...
4. **Heurística de domínio é textual** — só `processa\` e `processa.com\` são reconhecidos. Cliente que tente `PROCESSA\guga` funciona (case-insensitive). Cliente que tente `processa.com.br\guga` cai em `AuthenticateLocal`. Smith deve replicar exatamente os dois prefixos legados, não generalizar para "qualquer domain prefix".
