import React, { Suspense, Component, useEffect, useMemo, type ReactNode, type ErrorInfo } from "react";
...
export function FederatedApp({
  ...
  name: string;
  loader: () => Promise<{ default: React.ComponentType<AppInstanceProps> }>;
  appProps: AppInstanceProps;
}) {
  useRemoteStylesheet(name);
  // lazy precisa de identidade estável por loader; o mesmo loader sempre devolve
  // o mesmo módulo, então memoizar por `name` é seguro e evita remontar tudo
  // a cada re-render do shell.
  const LazyComponent = useMemo(() => React.lazy(loader), [name]); // eslint-disable-line react-hooks/exhaustive-deps
  return (
    <FederatedErrorBoundary name={name}>
      <Suspense fallback={<LoadingFallback />}>
        <LazyComponent {...appProps} />
      </Suspense>
