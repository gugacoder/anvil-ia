...
// Embarcado dentro do shell via <iframe src="/so/chat/">. Same-origin -> cookie
...
export function App() {
  ...
  const [sessionId, setSessionId] = useState<string | null>(() =>
    localStorage.getItem("hub.chat.session"),
  );

  useEffect(() => {
    if (sessionId) localStorage.setItem("hub.chat.session", sessionId);
    else localStorage.removeItem("hub.chat.session");
  }, [sessionId]);
